...
df_atual_df = os.path.join(dataset_patch, 'Perfil-Racao-Alho_resample.csv')
...
df_resampled['Class'] = y_resampled_labels
df_resampled['TIME'] = np.resize(time_column.values, df_resampled.shape[0])

columns_order = ['TIME'] + [col for col in df_resampled.columns if col != 'TIME' and col != 'Class'] + ['Class']
df_resampled = df_resampled[columns_order]

df_resampled.to_csv(os.path.join(dataset_patch, 'Perfil-Racao-Alho_resample.csv'), index=False)
